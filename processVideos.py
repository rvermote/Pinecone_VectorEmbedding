...
#Setting constants, definitions and loading in models
...
for i in range(1,NUM_OF_VIDEOS+1):
    ...
    for i in range(0,len(tokenInput),BATCH_SIZE):
        ...
        ids = list(map(lambda x:str(vidIndex)+"-"+str(i)+"-"+str(x["start"]), batch))
        #get embeddings using the sentence transformer
        embeddings = retriever.encode(list(map(lambda x:x["text"],batch))).tolist()
        metadata = list(map(lambda x:{"text":x["text"],"video_id":vidIndex,"start":int(x["start"]),"end":int(x["end"])},batch))
